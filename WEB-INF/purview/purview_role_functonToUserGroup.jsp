...
String function_ID=StringUntil.getParameter(request,"function_ID",""); // 二级 菜单编号
...
String onclickUrl="purview_role_functonToUserGroup";                           // 菜单跳转 地址
...
String GroupRole=StringUntilDatabase.getRoleIdsByGroup(GroupId);
								...
								// 获取Iframe 中的对象
                                  ...
                                  function role_user()
                                 {
                                    // 获取 选中的 节点
                                      ...
                                      $.get("RoleAction_GroupTorole.action?<%=StringUntil.menuUrl(function_ID)%>&pageId="+new Date().getTime(), {  
                                        "groupRole.groupId":<%=GroupId%>,    ///   用户组id id
                                   	   "groupRole.roleIds":Fids                 // 节点id 集合
                                              },function(date){
                                                       
                                                      if(date.length>15)
                                                      {
                                                          alert("<%=Constants.errorPurviewText%>");
                                                       }else
                                                      alert(date);   
					...
					<table width=100% border=0 align=center cellpadding=0 cellspacing=0>
                      ...
                      onclickUrl   点击节点 跳转的 url
									...
									<iframe id="iframeRole"
										src="PurviewAction_getListToMenu.action?<%=StringUntil.menuUrl(function_ID)%>&menuLeftID=<%=menuLeftID%>
										                                        &groupType=<%=groupType%>
										                                      
										                                        &checkbox=,'checkbox'  
										                                        &icons=false
										                                        &onclickUrl=<%=onclickUrl %>
										                                        &contextmenu=false&checkedNodeID=<%=GroupRole%>
														...
														<table border=0 cellspacing=0 cellpadding=2 width=100% height="100%"
					                                        ...
					                                        <td style="padding-right: 20px; margin: 0px; height: 100%"  valign="top"  nowrap>
																...
																<iframe id="ifRameFunction"
																	src="PurviewAction_getListToMenu2.action?<%=StringUntil.menuUrl(function_ID)%>&pageId=<%=new Date().getTime() %>
