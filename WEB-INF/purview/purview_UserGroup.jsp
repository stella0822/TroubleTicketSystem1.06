...
String groupType="10"; // 下拉树 类型
String onclickUrl="NoChickUserAction_getList_Menu"; // 菜单跳转 地址

// 页面左边菜单编号
String menuLeftID=StringUntil.getParameter(request,"menuLeftID","");
// 返回的错误 信息
String s=StringUntil.getParameter(request,"stateText","");

List userList=(List)StringUntil.getParameterToobject(request,"userList",null);
if(userList==null)
{
	WebApplicationContext wac=WebApplicationContextUtils.getWebApplicationContext(this.getServletContext());
     UserAction sch= (UserAction)wac.getBean("chickUserAction");
    userList= sch.NoChickUserAction_getList(null,menuLeftID);
}
													...
													<td>
														...
														<table border=0 cellspacing=0 cellpadding=2 width=100%
															...
															<tr>
																...
																<td
																	style="padding-right: 20px; margin: 0px;"
																	nowrap>
																	...
																	<table border="0" cellpadding="0" cellspacing="0">
																		<tr >
																			...
																			<script type="text/javascript">
																			 ...
																			 window.location =document.URL.split('?')[0]+'?menuLeftID=<%=menuLeftID%>&pageURLid='+new Date().getTime();  
																				...
																				<div functionId="function_159" style="display: inline;" >【 <a href="#" onclick="getUserBygroupID();return false;">包含用户</a>】</div>
																				
																				<div functionId="function_157" style="display: inline;" >【 <a href="#" onclick="addGroupRole('<%=menuLeftID%>');return false" >组权限</a>】</div>  
																				<script type="text/javascript">
																		         function  addGroupRole(id)
																		         {
																		        	 ...
																		        	 var tdVal=$("td[id='"+id+"']");
	                                                                                 var url="purview_role_functonToUserGroup.action?<%=StringUntil.menuUrl("157")%>&menuLeftID=<%=menuLeftID%>"+
	      										                                             "&groupType=<%=groupType%>"+
																		...
																		<tr >
																			<td style="margin: 2px">
																				<div functionId="function_116" style="display: inline;" > <input   class="crmbutton small delete" type="button"
																					value="删除" onClick="delUser();" /></div>
																				&nbsp;&nbsp;
																				<script type="text/javascript">
																				/// 打开添加用户 窗口
																	         function  addUser()
																	         ...
																	         function  addRole(id)
																	         {
																	        	 ...
																	        	 var tdVal= $("td[id='"+id+"']");
                                                                                 var url="purview_role_functonTouser.action?<%=StringUntil.menuUrl("158")%>&menuLeftID=<%=menuLeftID%>"+
      										                                             "&groupType=<%=groupType%>"+
  										                                                 "&userId="+id+"&userName="+tdVal.eq(1).attr("val");
																			   ...
																			   function  updateUser(id)
																		         {
																		        	var tdVal= $("td[id='"+id+"']");
																		        	 ...
																		        	 $("#updateUser #userEmail").val(tdVal.eq(3).attr("val"));
																		        	 $("#updateUser #department option[value='"+tdVal.eq(5).attr("val")+"']").attr("selected",true);
																		        	 $("#updateUser #post option[value='"+tdVal.eq(4).attr("val")+"']").attr("selected",true);
																		        	 $("#updateUser #state option[value='"+tdVal.eq(7).attr("val")+"']").attr("selected",true);
																		        	 $("#updateUser #tel").val(tdVal.eq(6).attr("val"));
																		        	 $('#mb').attr('style','z-index: 4;');
																		        	 $('#updateUser').show(200);
																			     }
																	           function delUser(id)
																	           {   
                                                                                    ...
                                                                                    window.location="NoChickUserAction_del.action?<%=StringUntil.menuUrl("116")%>&userID="+id;
																		       }
												                               </script>
																				 <div functionId="function_114" style="display: inline;" >    <a href="#" onclick="addUser();return false;"> <img
																				    ...
																				    <div functionId="function_159" style="display: inline;" >	用户姓名：
																				   ...
																				   <script type="text/javascript">
																				      function selectByReadName()
																				      {
                                                                                          ...
                                                                                          window.location="<%=basePath%>NoChickUserAction_getList_Menu.action?<%=StringUntil.menuUrl("159")%>&menuLeftID=<%=menuLeftID%>&user.readName="+readName;
																	 ...
																	 for(int i=0;i<str.length;i++)
																	  {
																		...
																		%>
																				<td>
																				     <div functionId="function_158" style="display: inline;" > <a href="#" onclick="addRole('<%=StringUntil.isNull(str[0])%>');return false;">权限</a> |</div>
																					 <div functionId="function_115" style="display: inline;" ><a href="#" onclick="updateUser('<%=StringUntil.isNull(str[0])%>');return false;">编辑</a> |</div>
																					 <div functionId="function_116" style="display: inline;" ><a href='#' onclick="delUser('<%=StringUntil.isNull(str[0])%>');return false;">刪除</a></div>
																				</td>
...
添加用户
 -->
		...
		<div id="addUser" class="mb3" align="center"
			...
			<form id="addUserForm" action="<%=basePath%>NoChickUserAction_addUser.action?<%=StringUntil.menuUrl("114")%>" method="post">
				<br />
				...
				<table id="firste_validate" style="top: 0px;" width="100%"
					...
					<tr height="30px">
						...
						<td>
							<input type="text" id="userName" name="user.loginName" value=""
								class=":required :ajax;NoChickUserAction_chickUserName.action?<%=StringUntil.menuUrl("160")%>" />
						</td>
					</tr>
					<tr height="30px">
						<td align="right">
					...
					<tr height="30px">
						...
						<td>
							<input id="mailaddr" type="text" name="user.userEmail" value=""
								class=":required" />
						</td>
					</tr>
					<tr height="30px">
		...
		--%><div id="updateUser" class="mb3" align="center"
			style="top: 20%; left: 40%; width: 400px;">
			<form id="updateUserForm" action="<%=basePath%>NoChickUserAction_updateUser.action?<%=StringUntil.menuUrl("115")%>" method="post">
				<br />
				<input type="hidden" id="userid" name="user.id" value="">
				<table id="firste_validate" style="top: 0px;" width="100%"
					height="100%" border="0" cellpadding="3" cellspacing="3">
					...
					<tr height="30px">
						...
						<td>
							<input type="text" id="userName" name="loginName" disabled="disabled" readonly="readonly" value=""
								 />
						</td>
					</tr>
					<tr height="30px">
		    ...
		    <div id="addRole" class="mb3" align="center"
			style="top: 10%; left: 20%; width: 800px;">
			      
			               自动 给value 为 【关闭】 的按钮 添加 清除 验证样式 事件 
			      
				<input class="delete" type="button" value="关闭"
					onclick="$('#addRole').hide(200);$('#mb').hide();" />
				&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp;
				<input id="deleteMenuSave" class="save" type="submit" value="保存" />
				
                      menuLeftID   需要展开 菜单的  id
                       groupType   左边菜单类型    1是用户组
                          menuID   头部菜单编号 用于权限控制和操作日志 
                      FunctionID   功能编号           用于权限控制和操作日志
                      onclickUrl   点击节点 跳转的 url
					   
									<iframe
										src='purview_role_functonTouser.action?<%=StringUntil.menuUrl("")%>&menuLeftID=<%=menuLeftID%>
										                                       &groupType=<%=groupType%>
		...
		返回错误信息
	     --%><div id="erroText_div" class="mb3" align="center"
           ...
           function errorCleas()
           {
         	  $('#erroText_div').hide();
         	  $('#mb').hide();
         	  var url_action=document.URL.split('?')[0];
         	  var url_parm=document.URL.split('?')[1];
         	      url_parm=url_parm.split("&");
         	      url_parm=url_parm[1]+"&"+url_parm[2];
         	      var url=url_action+"?"+url_parm;
         	 
         	  window.location =url+'&pageURLid='+new Date().getTime();  
           }
           </script>
			<input class="delete" type="button" value="关闭"
				   onclick="errorCleas();" />
					 ...
					 <div id="iframe" style="display: none;">
									...
									<iframe src="PurviewAction_getListToMenu.action?<%=StringUntil.menuUrl("156") %>&groupType=<%=groupType%>&menuLeftID=<%=menuLeftID%>&onclickUrl=<%=onclickUrl%>"
