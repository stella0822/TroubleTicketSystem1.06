...
$(function(){
	//必填项加红*
    $("input[class*=':required'],select[class=':required']").after("<span> *</span>");
    //给关闭 按钮 添加 样式 还原事件
    $("form input[value*='关闭']").click(function(){
    	 $("span[class='failmsg failformcss']").remove();
    	 $(".failformcss,.rightformcss").attr("class","inputDefault");
    	
    });
    $("form input[type='submit']").click(function(){
    	　
   	  if($("form span[class='failmsg failformcss']").length>0){
   		  return false;
   	  };
...
// 封装 ajax 验证方法
function ajaxChikByp(val,url)
{
	                 
    $.post(url,{'date':val},function(data){
...
Vanadium.addValidatorTypes([
    ...
    ['alphCN',function(v){
    	  return Vanadium.validators_types['empty'].test(v)|| /^[\u4e00-\u9fa5]+$/gi.test(v)
    },'请输入中文！'],
	//邮箱验证
    ['email', function (v) {
